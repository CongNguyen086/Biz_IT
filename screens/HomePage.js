...
export default function HomePage(props) {
  ...
  getTimeRecommendation = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.1.15:3000/gettimerecommendationdeal?userId=${token}`);
  ...
  getRecommendation = async () => {
    try {
      const token = await AsyncStorage.getItem('@userToken');
      const response = await fetch(`http://192.168.1.15:3000/getrecommendeddeal?userId=${token}`);
      const jsonData = await response.json();
